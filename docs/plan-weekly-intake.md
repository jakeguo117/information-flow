...
主文件：`📋 Digests/{YYYY-Wnn}/intake.md`（一篇累积，不是每天一个 Digest）。讨论时按上海当天自己认周，不用人口报 W38。
...
- `weekly_intake.resolve_intake`：上海当天 ISO 周的 `intake.md`；没有就回落到已有的最近一周。

## Skill

- intake：自己认周。先读最近周记和已挂上的老项目，再从那一周 intake 挑一条呼应。Yale 式一次一问。禁止整周附录出文。记下则 `[x]` + 带引用 reflection；有引用才写 `📖 Resources/concepts/`。
